<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    <!-- Liens spécifiques au gestionnaire -->
                    @if (Auth::user()->role === 'gestionnaire')
                        <x-nav-link :href="route('livres')" :active="request()->routeIs('livres.index')">
                            {{ __('Gérer les livres') }}
                        </x-nav-link>
                    ...
                    @elseif (Auth::user()->role === 'client')
                        ...
                        <x-nav-link :href="route('livres.client')" :active="request()->routeIs('livres.index')">
                            {{ __('Parcourir les livres') }}
                        </x-nav-link>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            <!-- Liens spécifiques au gestionnaire -->
            @if (Auth::user()->role === 'gestionnaire')
                <x-responsive-nav-link :href="route('livres')" :active="request()->routeIs('livres.index')">
                    {{ __('Gérer les livres') }}
                </x-responsive-nav-link>
                <x-responsive-nav-link :href="route('commandes')" :active="request()->routeIs('commandes.index')">
                    {{ __('Voir les commandes') }}
                </x-responsive-nav-link>
                <x-responsive-nav-link :href="route('statistiques')" :active="request()->routeIs('statistiques.index')">
                    {{ __('Statistiques') }}
                </x-responsive-nav-link>
            @elseif (Auth::user()->role === 'client')
                <!-- Liens spécifiques au client -->
